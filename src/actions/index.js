...
export function getCurrentPageUrl() {
	return (dispatch, getState) => {
		...
		gettingActiveTab.then((tabs) => {
			...
			browser.tabs.sendMessage(tabs[0].id, {"message": "get_current_page_info"}, function(result) {
				if (result === null) {
					return
				}
				dispatch(receiveCurrentPagePrice(country, result))
			})
...
export function getCountryPrice(productID, country) {
	return (dispatch, getState) => {
		...
		.catch(function(err) {
			console.log('There has been a problem with fetching price for other amazon markets: ' + err.message)
		})
	}
}